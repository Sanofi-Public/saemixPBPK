...
initialiseMainAlgo<-function(saemix.data,saemix.model,saemix.options) {
  ...
  if(length(saemix.data["name.covariates"])>0) 
    Mcovariates<-data.frame(id=rep(1,N),temp[,2:dim(temp)[2]])
  else 
  ...
  for(j in 1:nb.parameters) {
    ...
    j1<-j2+1
    if(length(jcov)<=1) mean.phi[,j]<-aj*lambdaj 
    else mean.phi[,j]<-aj%*%lambdaj
  ...
  XM <- XM %>% mutate(id=chdat["dataM"][["IdM"]])
  ...
  while (ltest.phi>0) {
    ...
    fpred <- NULL
    pb <- progress::progress_bar$new(format = " [:bar] :percent // id :what  ", total = nrow(phiM), width = 60)
    pb$tick(0)
    Sys.sleep(1)
    for (i in seq_len(nrow(phiM))) {
      pb$tick(tokens = list(what = i))
      fpred<-c(fpred, structural.model(psiM[i,], IdM[i], XM[i,]))
    }
    pb$terminate() 
