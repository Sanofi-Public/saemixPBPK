...
saemix<-function(model,data,control=list()) {
  ...
  #                      max = saemix.options$nbiter.tot, # Maximum value of the progress bar
  #                      style = 3,    # Progress bar style (also available style = 1 and style = 2)
  #                      #width = saemix.options$nbiter.tot/2,   # Progress bar width. Defaults to getOption("width")
  #                      char = "=")
  
  pb <- progress::progress_bar$new(format = " [:bar] :percent // iteration :what  ", total = saemix.options$nbiter.tot, width = 60)
  # Running the algorithm
  cat(" // SAEM iterations // \n")
  pb$tick(0)
  Sys.sleep(1)
  for (kiter in 1:saemix.options$nbiter.tot) { # Iterative portion of algorithm
    pb$tick(tokens = list(what = kiter))
  ...
  if(saemix.options$print) print(saemixObject,digits=4)
  ...
  if(saemix.options$save.graphs) {
    ...
    if(!inherits(xtry,"try-error")) {
      ...
      if (length(saemixObject@results@name.random)>1)
