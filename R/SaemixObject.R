...
saemixControl<-function(map=TRUE,fim=TRUE,ll.is=TRUE,ll.gq=FALSE,nbiter.saemix=c(300,100), nbiter.sa=NA, nb.chains=1,fix.seed=TRUE,seed=23456,
                        nmc.is=5000, nu.is=4, r.is=1, H.is=0.5, S.is=0.05, W.is=4000, clean.is=1000, MM.is=100, mlx.project=NULL, print.is=FALSE, 
                        nbdisplay=100, displayProgress=FALSE, nbiter.burn=5,nbiter.map=5, nbiter.mcmc=c(1,1,1,0),proba.mcmc=0.4,stepsize.rw=0.4,rw.init=0.5,
                        alpha.sa=0.97,  nnodes.gq=12,nsd.gq=4,maxim.maxiter=100,nb.sim=1000,nb.simpred=100, ipar.lmcmc=50,ipar.rmcmc=0.05, print=FALSE, save=TRUE, 
  ...
  list(map=map,fim=fim,ll.is=ll.is,ll.gq=ll.gq,nbiter.saemix=nbiter.saemix, nbiter.sa=nbiter.sa, nbiter.burn=nbiter.burn, nbiter.map=nbiter.map,nb.chains=nb.chains,
       fix.seed=fix.seed,seed=seed, 
       nmc.is=nmc.is,nu.is=nu.is,r.is=r.is,print.is=print.is, H.is=H.is, S.is=S.is, W.is=W.is, clean.is=clean.is, MM.is=MM.is, mlx.project=mlx.project,
       nbdisplay=nbdisplay,displayProgress=displayProgress,
...
saemix.predict<-function(object, type=c("ipred", "ypred", "ppred", "icpred")) {
  ...
  id <- seq_len(object@data@N)
  ppred<-object["model"]["model"](as.matrix(psiM),id,xind)
  ...
  if(length(intersect(c("ipred","icpred"),type))>0) {
    ...
    if("ipred" %in% type) {
      ipred<-object["model"]["model"](as.matrix(saemix.res["map.psi"]),id,xind) # Predictions with MAP
      #ipred<-object["model"]["model"](as.matrix(saemix.res["map.psi"]),index,xind) # Predictions with MAP
      ires<-yobs-ipred
    ...
    if("icpred" %in% type) {
      ...
      icond.pred<-object["model"]["model"](psiM,id,xind) # Predictions with Conditional mean
