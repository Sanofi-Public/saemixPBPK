...
setClass(
  ...
  representation=representation(
    modelPKSim="function", 		# name of model function
...
setMethod(
  ...
  definition=function(.Object,model,modelPKSim,description,modeltype,psi0, name.response, name.sigma, transform.par,fixed.estim, error.model,covariate.model,covariance.model,omega.init,error.init, name.modpar, verbose=TRUE){
...
setMethod(
  ...
  definition = function (x,i,j,drop ){
    switch (EXPR=i,
            ...
            "modelPKSim"={return(x@modelPKSim)},
...
saemixModel<-function(model, modelPKSim, psi0, description="",modeltype ="structural", name.response="", name.sigma=character(), error.model=character(), transform.par=numeric(),fixed.estim=numeric(),covariate.model=matrix(nrow=0,ncol=0), covariance.model=matrix(nrow=0,ncol=0),omega.init=matrix(nrow=0,ncol=0),error.init=numeric(), name.modpar=character(), verbose=FALSE) {
  ...
  xmod<-try(new(Class="SaemixModel",model=model,modelPKSim=modelPKSim,description=description , modeltype=modeltype,psi0=psi0, name.response=name.response, name.sigma=name.sigma, error.model=error.model, transform.par=transform.par,fixed.estim=fixed.estim, covariate.model=covariate.model,covariance.model=covariance.model, omega.init=omega.init,error.init=error.init,name.modpar=name.modpar))
