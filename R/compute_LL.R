...
llis2.saemix<-function(saemixObject) {
  ...
  MM <- saemixObject["options"]$MM.is
  NMC <- saemixObject["options"]$nmc.is
  nu <- saemixObject["options"]$nu.is
  W <- saemixObject["options"]$W.is
  H <- saemixObject["options"]$H.is
  S <- saemixObject["options"]$S.is
  k.clean <- saemixObject["options"]$clean.is
  mlx.dir <- gsub(".mlxtran", "", saemixObject["options"]$mlx.project)
  cmin <- 1
  
  KM<-round(NMC/MM)
  
  N <- saemix.data["N"]
 # IdM<-rep(c(0:(MM-1)),each=saemix.data["ntot.obs"])*N+ rep(saemix.data["data"][,"index"],MM)
  IdM <- rep(1:saemix.data["N"],MM)
  yM<-rep(yobs,MM)
  XM<-do.call(rbind,rep(list(xind), MM))
  
  io<-matrix(0,nrow=N,ncol=max(saemix.data["nind.obs"]))
  for(isuj in 1:N)
  ...
  rvar=saemixObject["options"]$r.is
  cond.var.phi1<-cond.var.phi[,i1.omega2,drop=FALSE]*rvar
 ...
 #   browser()
    randEff <- file.path(mlx.dir, "IndividualParameters/simulatedRandomEffects.txt")
    re=read.table(randEff, header=T, sep=",")
    sd.re <- re %>% select(-rep) %>% group_by(id) %>% summarise_all("sd")%>% select(-id) 
    Rsd <- NULL
    for (j in 1:nphi1) {
      sdj <- sd.re[,j][[1]]
      Rsd <- cbind(Rsd, (1*(sdj-min(sdj)) + sqrt(rvar)*(max(sdj)-sdj))/(max(sdj)-min(sdj)))
    }
    re <- (re %>% arrange(rep, id))[,2+(1:nphi1)]
    list.cov <- list()
    for (i in 1:N) {
      #      browser()
      #      list.cov[[i]] <- cov(re[seq(i, nrow(re), by=N),])*rvar
      list.cov[[i]] <- diag(Rsd[i,])%*%cov(re[seq(i, nrow(re), by=N),])%*%diag(Rsd[i,])
 #     list.cov[[i]] <- diag(diag(list.cov[[i]]))
    }
  }
  ...
  m.eLL <- 0
  j.clean <- 0
  while (!test | !test.clean) {
    k <- k+1
    if (is.null(randEff)) {
      r<-trnd.mlx(nu,N*MM,nphi1)
      phiM1<-mtild.phiM1+stild.phiM1*r
      pitild.phi1<-rowSums(log(tpdf.mlx(r,nu)))
      e3<-matrix(pitild.phi1,nrow=N,ncol=MM)- matrix(rep(0.5*rowSums(log(cond.var.phi1)),MM),ncol=MM)
    } else {
      ei <- e3 <- NULL
      for (i in (1:N)) {
        #        if (k==73 & i==23) browser()
        test.r <- F
        while(!test.r) {
          ri <- rmvt(MM, sigma=list.cov[[i]], df=nu, delta=cond.mean.phi[i,i1.omega2], type="shifted")
          maxi <- apply(abs(ri), 2, max)
          qi <- apply(abs(ri), 2, quantile, 0.9)
          if (max(maxi/qi)<50)    test.r <- T
        }
        pi <- dmvt(ri, sigma=list.cov[[i]], df=nu, delta=cond.mean.phi[i,i1.omega2], type="shifted", log = TRUE)
        ei <- cbind(ei, ri)
        e3 <- rbind(e3, pi)
      }
      phiM1 <- matrix(t(ei), ncol=nphi1, byrow=T)
    }
    
    dphiM<-phiM1-mean.phiM1
    d2<-(-0.5)*(rowSums(dphiM*(dphiM%*%IOmega.phi1)) + c2)
    e2<-matrix(d2,nrow=,ncol=MM)
    
    phiM[,i1.omega2]<-phiM1
    psiM<-transphi(phiM,saemix.model["transform.par"])
    fM<-saemix.model["model"](psiM,IdM,XM)
    g<-error(fM,pres,XM$ytype)
    DYF[ind.ioM] <- -0.5*((yM-fM)/g)**2 - log(g) - 0.5*c1
    e1<-matrix(colSums(DYF),nrow=N,ncol=MM)
    
    llk<-e1+e2-e3
    # if (length(which(is.na(llk)))>0) browser()
    LLk <- cbind(LLk, llk)
    if (((k*MM) %% k.clean) == 0) {
      j.clean <- j.clean + 1
      Sj <- log((j.clean-1)+exp(S))-log(j.clean)
      #    Sj <- S
      test.clean <- T
      # if (k==80) browser()
      r.clean <- cleanLL(LLk, S.diff=Sj)
      #      print(r.clean$S)
      LLk <- r.clean$LL
      Lk <- exp(LLk) 
      ofv <- NULL
      m.eLL <- 0
      for (m in 1:k) {
        mj <- rowMeans(Lk[,((m-1)*MM+1):(m*MM)])
        m.eLL <- m.eLL + 1/m*(mj - m.eLL)
        ofv <- c(ofv, -2*sum(log(m.eLL)))
      }
      
    } else {
      Lk <- exp(LLk[,((k-1)*MM+1):(k*MM)])
      m.eLL <- m.eLL + 1/k*(rowMeans(Lk) - m.eLL)
      ofv <- c(ofv, -2*sum(log(m.eLL)))
    }
    if (k >= W) {
      yk <- ofv[(k-W+1):k]
      # print(c(k, min(yk), max(yk), H))
      # if (k==73)  browser()
      if (max(yk, na.rm=T)-min(yk, na.rm=T) < H  & abs(cor(yk, 1:W, use="pairwise.complete.obs")) < cmin)
        test <- T
      if (k>= KM)
        test <- T
    }
  ...
  if(sum(is.na(y1))) { # Johannes
    ...
    if (saemixObject["options"]$print.is) 
      try(plot(x1,y1,type="l",xlab="Size of the Monte-Carlo sample", ylab="'-2xLog-Likelihood",main="Estimation of the log-likelihood"))
  }
  zk <- (max(yk)+min(yk))/2
  seg <- data.frame(x1=rep((k-W+1)*MM, 2), x2=rep(k*MM, 2), y1=zk+c(-H/2, H/2), y2=zk+c(-H/2, H/2) )
  
  #  saemixObject["results"]["seg.is"]<-seg
  saemixObject["results"]["LL"]<-c(LL)
  saemixObject["results"]["ll.is"]<-LL[KM]
  saemixObject["results"]["aic.is"]<- -2*LL[KM] + 2*saemixObject["results"]["npar.est"]
  saemixObject["results"]["bic.is"]<- -2*LL[KM] + log(N)*saemixObject["results"]["npar.est"]
  saemixObject["results"]["bic.covariate.is"]<- -2*LL[KM] + log(N)*saemixObject["results"]["nbeta.random"]+log(sum(saemixObject["data"]["nind.obs"]))*saemixObject["results"]["nbeta.fixed"]
  return(saemixObject)
}

cleanLL <- function(LL, S.diff=0.2) {
  N <- nrow(LL)
  K <- ncol(LL)
  sL <- t(apply(exp(LL), 1, sort))
  oLL <- t(apply(LL, 1, order))
  # browser()
  ofv <- -2*log(t(apply(sL, MARGIN=1, cummean)))
  S <- rep(0,N)
  stat <- NULL
  for (i in (1:N)) 
    stat <- rbind(stat, diff(ofv[i,])/ofv[i,1:(K-1)]*(1:(K-1)))
  if (is.na(S.diff)) {
    delta <- 5
    if (!is.null(delta)) {
      S.diff <- Inf
      sstat <- sort(stat)
      iS <- which(diff(sstat) > delta)
      if (length(iS)>0) 
        S.diff <- -sstat[max(iS)+1]
    }
  }
  for (i in (1:N)) {
    stati <- stat[i,]
    i.ofv <- which(stati > -S.diff)
    if (length(i.ofv)>0) {
      i.max <- max(i.ofv) + 1
      if (i.max < K) {
        for (k in ((i.max+1):K)) {
          #   Sk <- S.diff
          Sk <- S.diff/(k-1)*ofv[i,k-1]
          LL[i,oLL[i,k]] <- log(k*exp(Sk/2) - (k-1)) - ofv[i, k-1]/2
...
llis.saemix<-function(saemixObject) {
  ...
  yM<-rep(yobs,MM)
  XM<-do.call(rbind,rep(list(xind), MM))
  XM <- XM %>% mutate(id=rep(c(0:(MM-1)),each=saemix.data["ntot.obs"])*saemix.data["N"]+ rep(saemix.data["data"][,"index"],MM))
  #XM<- rep(saemix.data["data"][,"index"],MM)
  ...
  randEff <- NULL
  #randEff <- "warf12/IndividualParameters/simulatedRandomEffects.txt"
  if (!is.null(randEff)) {
    re=read.table(randEff, header=T, sep=",")
    re <- (re %>% arrange(rep, id))[,2+(1:nphi1)]
    list.cov <- list()
  ...
  cat(" // LL Importance Sampling // \n")
  kmin<-min(10,ceiling(KM/4))
  pb <- progress::progress_bar$new(format = " [:bar] :percent // Iteration :what  ", total = KM, width = 60)
  pb$tick(0)
  Sys.sleep(0.5)
  for(km in 1:KM) {
    ...
    # log(tpdf.mlx(r[1,],nu)) - 0.5*log(cond.var.phi1[1,])
    # dmvt((mtild.phiM1+stild.phiM1*r)[1,], sigma=diag(cond.var.phi1[1,]), df=nu, delta=mtild.phiM1[1,], type="shifted", log = TRUE)
    
    phiM[,i1.omega2]<-phiM1
    psiM<-transphi(phiM,saemix.model["transform.par"])
    fM<-saemix.model["model"](psiM,IdM,XM)
    for(i in idx.exp) fM[saemix.data["data"][,"ytype"]==i]<-log(cutoff(fM[saemix.data["data"][,"ytype"]==i]))
    #		if(saemix.model["error.model"]=="exponential")
    #			fM<-log(cutoff(fM))
    ...
    # 
    #if (km==128)  browser()  # e3[11, 37]  r[1163]
    # i = 11 ;  j=35:39
    # i = 28 ;  j=55:59
    # e3m <- matrix(pitild.phi1,nrow=N,ncol=MM)
    # rbind(e1[i,j], e2[i,j],-e3[i,j],-e3m[i,j], sume[i,j])
...
llgq.saemix<-function(saemixObject) {
  ...
  if (length(mlx.dir)==0) {
    ...
    # randEff <- file.path(mlx.dir, "IndividualParameters/simulatedRandomEffects.txt")
    # re=read.table(randEff, header=T, sep=",")
    indparam <- read.table(file.path(mlx.dir, "IndividualParameters/simulatedIndividualParameters.txt"), header=T, sep=",")
    mean.phi <- indparam %>% select(-rep) %>% group_by(id) %>% summarise_all(function(x) mean(log(x)))%>% select(-id) 
    min.phi  <- indparam %>% select(-rep) %>% group_by(id) %>% summarise_all(function(x) min(log(x))) %>% select(-id) 
    max.phi  <- indparam %>% select(-rep) %>% group_by(id) %>% summarise_all(function(x) max(log(x))) %>% select(-id) 
    #sd.re <- re %>% select(-rep) %>% group_by(id) %>% summarise_all("sd")%>% select(-id) 
    # mean.re <- re %>% select(-rep) %>% group_by(id) %>% summarise_all("mean")%>% select(-id) 
    # min.re <- re %>% select(-rep) %>% group_by(id) %>% summarise_all("min")%>% select(-id) 
    # max.re <- re %>% select(-rep) %>% group_by(id) %>% summarise_all("max")%>% select(-id) 
    xmin <- mean.phi + (min.phi - mean.phi)*nsd.gq
    xmax <- mean.phi + (max.phi - mean.phi)*nsd.gq
  } 
  
  
  a<-as.matrix((xmin+xmax)/2)
  b<-as.matrix((xmax-xmin)/2)
  log.const<-0
  idx.exp<-which(saemixObject["model"]["error.model"]=="exponential")
  if(length(idx.exp)>0)
    #	if(saemixObject["model"]["error.model"]=="exponential")
  ...
  for (j in 1:nx) {
    ...
    if(saemixObject["model"]["modeltype"]=="structural"){
      fM<-saemixObject["model"]["model"](psi, saemix.data["data"][,"index"], xind)
      for(i in idx.exp) fM[saemix.data["data"][,"ytype"]==i]<-log(cutoff(fM[saemix.data["data"][,"ytype"]==i]))
      g<-error(fM,pres,saemix.data["data"][,"ytype"])
      DYF[ind.io] <- -0.5*((yobs-fM)/g)**2 - log(g)
      ly<-colSums(DYF)
    } else {
      fM<-saemixObject["model"]["model"](psi, saemix.data["data"][,"index"], xind)
      DYF[ind.io] <- fM
